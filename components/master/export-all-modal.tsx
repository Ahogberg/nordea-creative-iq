...
export function ExportAllModal({
  ...
  useEffect(() => {
    if (open) {
      setSelectedFormats(new Set(variants.map((v) => v.format)));
      setJob(null);
  ...
  const handleExport = async () => {
    ...
    try {
      const filteredVariants = variants.filter((v) =>
        selectedFormats.has(v.format)
      );
      const res = await fetch("/api/master/export", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ variants: filteredVariants }),
      });
      ...
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-md mx-4">
          ...
          {!job && (
            <>
              <p className="text-sm text-nordea-text-secondary mb-4">
                Välj format att exportera
              </p>
              <div className="grid grid-cols-2 gap-2 mb-4">
                {variants.map((v) => {
                  const aspect = FORMATS[v.format];
                  const active = selectedFormats.has(v.format);
                  return (
                    <button
                      key={v.format}
                      type="button"
                      onClick={() => toggleFormat(v.format)}
              ...
              <p className="text-xs text-nordea-text-tertiary">
                Estimerad renderingstid: ~{selectedFormats.size * 30}s
              </p>
            </>
          )}
            ...
            <div className="py-4">
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-sm font-medium text-nordea-text">
                  {job.status === "pending"
                    ? "Förbereder…"
                    : `Renderar ${selectedFormats.size} format · ${job.progress}%`}
                </span>
              </div>
          ...
          {job && job.status === "done" && job.download_url && (
            <div className="py-4 text-center">
              ...
              <p className="text-sm font-medium text-nordea-text mb-3">
                {selectedFormats.size} format klara att ladda ner
              </p>
              <a
        ...
        {!job && (
          <div className="flex items-center justify-end gap-2 p-5 border-t border-nordea-hairline">
            ...
            >
              <Download className="w-4 h-4" />
              Exportera {selectedFormats.size} format
            </button>
          </div>
        )}
