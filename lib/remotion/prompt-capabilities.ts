...
- "canvas": fri komposition med egen TSX-kod — se ILLUSTRATIONER OCH FRI ANIMATION nedan. Fält: tsxCode, description, layers? (rörelse som keyframes), och för illustrationsscener: headline?, subtitle?, illustrationLayout?: "illustration-top"|"illustration-bottom", illustrationHeightPercent? (20–70, standard 48).
...
- Illustrationskit (valfritt att använda, fritt att kombinera med egen SVG):
  ...
  Rörelse i koden (returnerar värden): fall(frame, start, dur, distance) → {y, opacity}; slideIn(frame, start, dur, [dx, dy]) → {x, y, opacity}; slideAlongIso(frame, start, dur, [dx, dy, dz], unit) → {x, y, opacity}; grow(frame, start, dur) → {scale, opacity}; progress(frame, start, dur) → 0..1; loop(frame, period, start?) → 0..1 upprepat; bob(frame, amplitude, period?); drawOn(frame, start, dur, length) → strokeDashoffset; transformAt(x, y, scale?) → SVG-transform; ease.out / ease.inOut.

LAGER OCH KEYFRAMES (föredra för objekt som rör sig):
- Rita objektet i koden inne i <Layer id="mynt1" origin={[cx, cy]}>…</Layer> (origin = punkten som skala och rotation sker runt, i ytans px; utelämna om bara x/y/opacitet animeras).
- Beskriv rörelsen i scenens "layers" — inte i koden:
  "layers": [ { "id": "mynt1", "name": "Mynt 1", "keyframes": { "y": [ { "t": 0.7, "v": -260 }, { "t": 1.3, "v": 0, "ease": "ease-out" } ], "opacity": [ { "t": 0.7, "v": 0 }, { "t": 0.9, "v": 1 } ] } } ]
- Egenskaper: x, y (px i 1080-bred designskala, förskjutning från där objektet ritats), scale (1 = naturlig storlek), rotation (grader), opacity (0–1). t = sekunder från scenens början. ease = kurvan fram till nästa keyframe: "ease-out" (standard), "ease-in-out", "ease-in", "linear". Före första keyframen gäller dess värde, efter sista likaså.
- Då kan användaren dra i keyframes i tidslinjen och du kan justera timing med små patchar: { "op": "replace", "path": "/scenes/0/layers/1/keyframes/y/0/t", "value": 1.0 }.
- Koden kan läsa ett lagers aktuella läge med useLayer("mynt1") → { x, y, scale, rotation, opacity }.
- Använd kodens rörelsehjälpare bara för sådant som inte är läge/skala/rotation/opacitet (t.ex. fönster som tänds, en linje som ritas upp) och för oändliga loopar.
